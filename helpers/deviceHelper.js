...
const adbPath = path.join(__dirname, '../platform-tools', 'adb.exe');
const client = adb.createClient({ bin: adbPath });
const deviceNameFile = path.join(__dirname, '../database/device-names.json');
const fs = require('fs');
...
const getDeviceName = async (device_id) => {
  try {
    const data = fs.readFileSync(deviceNameFile, 'utf-8');
    const map = JSON.parse(data);
    return map[device_id] || device_id;
  } catch (err) {
    console.warn(`⚠️ Không đọc được tên thiết bị từ file: ${err.message}`);
    return device_id;
  }
}
...
module.exports = { getDeviceModel, getDeviceName, checkDeviceFHD, checkFontScale, checkWMDensity };